...
function lineAuthorOut(fileName: string): string[] {
  return [
    `docs/assets/screenshots/${fileName}`,
...
async function shoot(
  ...
  const screenshot = await locator.screenshot({ scale: "device" });
  for (const output of Array.isArray(outputs) ? outputs : [outputs]) {
    fs.writeFileSync(out(output), screenshot);
    console.log(`  ✓ ${output}`);
  }
}
...
test.describe("Line Author – settings panels", () => {
  test("activate toggle → line-author-activate.png", async ({ page }) => {
    await shoot(page, lineAuthorScenarios.renderLineAuthorActivate(), lineAuthorOut("line-author-activate.png"));
  });

  test("follow movement config → line-author-follow-config.png", async ({ page }) => {
    await shoot(page, lineAuthorScenarios.renderLineAuthorFollowConfig(), lineAuthorOut("line-author-follow-config.png"));
  });

  test("hash + name + date config → line-author-commit-hash-full-name-config.png", async ({ page }) => {
    await shoot(page, lineAuthorScenarios.renderLineAuthorHashNameConfig(), lineAuthorOut("line-author-commit-hash-full-name-config.png"));
  });

  test("custom date config → line-author-custom-dates-config.png", async ({ page }) => {
    await shoot(page, lineAuthorScenarios.renderLineAuthorCustomDatesConfig(), lineAuthorOut("line-author-custom-dates-config.png"));
  });

  test("timezone config → line-author-tz-config.png", async ({ page }) => {
    await shoot(page, lineAuthorScenarios.renderLineAuthorTzConfig(), lineAuthorOut("line-author-tz-config.png"));
  });

  test("color config → line-author-color-config.png", async ({ page }) => {
    await shoot(page, lineAuthorScenarios.renderLineAuthorColorConfig(), lineAuthorOut("line-author-color-config.png"));
  });

  test("text color config → line-author-text-color.png", async ({ page }) => {
    await shoot(page, lineAuthorScenarios.renderLineAuthorTextColor(), lineAuthorOut("line-author-text-color.png"));
  });
});
...
test.describe("Line Author – editor gutters", () => {
  test("default gutter → line-author-default.png", async ({ page }) => {
    await shoot(page, lineAuthorScenarios.renderLineAuthorDefault(), lineAuthorOut("line-author-default.png"));
  });

  test("hash + full name → line-author-commit-hash-full-name.png", async ({ page }) => {
    await shoot(page, lineAuthorScenarios.renderLineAuthorHashFullName(), lineAuthorOut("line-author-commit-hash-full-name.png"));
  });

  test("natural language dates → line-author-natural-language-dates.png", async ({ page }) => {
    await shoot(page, lineAuthorScenarios.renderLineAuthorNaturalLanguageDates(), lineAuthorOut("line-author-natural-language-dates.png"));
  });

  test("custom date format → line-author-custom-dates.png", async ({ page }) => {
    await shoot(page, lineAuthorScenarios.renderLineAuthorCustomDates(), lineAuthorOut("line-author-custom-dates.png"));
  });

  test("timezone UTC → line-author-tz-utc0000.png", async ({ page }) => {
    await shoot(page, lineAuthorScenarios.renderLineAuthorTzUtc(), lineAuthorOut("line-author-tz-utc0000.png"));
  });

  test("timezone author local → line-author-tz-author-local.png", async ({ page }) => {
    await shoot(page, lineAuthorScenarios.renderLineAuthorTzAuthorLocal(), lineAuthorOut("line-author-tz-author-local.png"));
  });

  test("timezone viewer +01:00 → line-author-tz-viewer-plus0100.png", async ({ page }) => {
    await shoot(page, lineAuthorScenarios.renderLineAuthorTzViewerPlus0100(), lineAuthorOut("line-author-tz-viewer-plus0100.png"));
  });

  test("text color muted → line-author-text-color-muted.png", async ({ page }) => {
    await shoot(page, lineAuthorScenarios.renderLineAuthorTextColorMuted(), lineAuthorOut("line-author-text-color-muted.png"));
  });

  test("text color normal → line-author-text-color-normal.png", async ({ page }) => {
    await shoot(page, lineAuthorScenarios.renderLineAuthorTextColorNormal(), lineAuthorOut("line-author-text-color-normal.png"));
  });

  test("whitespace: before changes → line-author-ignore-whitespace-before.png", async ({ page }) => {
    await shoot(page, lineAuthorScenarios.renderLineAuthorIgnoreWhitespaceBefore(), lineAuthorOut("line-author-ignore-whitespace-before.png"));
  });

  test("whitespace: preserved → line-author-ignore-whitespace-preserved.png", async ({ page }) => {
    await shoot(page, lineAuthorScenarios.renderLineAuthorIgnoreWhitespacePreserved(), lineAuthorOut("line-author-ignore-whitespace-preserved.png"));
  });

  test("whitespace: ignored → line-author-ignore-whitespace-ignored.png", async ({ page }) => {
    await shoot(page, lineAuthorScenarios.renderLineAuthorIgnoreWhitespaceIgnored(), lineAuthorOut("line-author-ignore-whitespace-ignored.png"));
  });

  test("untracked lines → line-author-untracked.png", async ({ page }) => {
    await shoot(page, lineAuthorScenarios.renderLineAuthorUntracked(), lineAuthorOut("line-author-untracked.png"));
  });

  test("follow: do not follow → line-author-follow-no-follow.png", async ({ page }) => {
    await shoot(page, lineAuthorScenarios.renderLineAuthorFollowNoFollow(), lineAuthorOut("line-author-follow-no-follow.png"));
  });

  test("follow: all commits → line-author-follow-all-commits.png", async ({ page }) => {
    await shoot(page, lineAuthorScenarios.renderLineAuthorFollowAllCommits(), lineAuthorOut("line-author-follow-all-commits.png"));
  });

  test("copy commit hash context menu → line-author-copy-commit-hash.png", async ({ page }) => {
    await shoot(page, lineAuthorScenarios.renderLineAuthorCopyHash(), lineAuthorOut("line-author-copy-commit-hash.png"));
  });
});
