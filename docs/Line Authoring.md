...
Line Authoring shows who last changed each line and when. It is based on [git blame](https://git-scm.com/docs/git-blame).

> [!info]
> The line author view works in Live Preview and Source mode. It does not run in Reading mode.

> [!info]
> Line Authoring is currently desktop-only.

> [!note]
> Static screenshots in this document are auto-generated by the Playwright suite in `screenshots/tests/screenshots.spec.ts`. Re-run `make docs-screenshots` to refresh both `docs/assets/line-author-*.png` and `docs/assets/screenshots/line-author-*.png`. GIF animations are manual recordings and must be updated by hand.
...
This displayed time is the same for all users.
...
## Follow lines across cut, copy, and paste

By default, each line shows the last commit, where it was changed.
This means that cut, copy, and paste operations will show the new commit,
...
## Soft asynchronous view updates
