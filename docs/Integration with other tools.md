...
1. To make `/opt/homebrew/bin/` available in Obsidian, add `/opt/homebrew/bin/` to the "Additional PATH environment variables paths" setting under "Advanced".
2. Restart Obsidian.
...
1. Run `which git-lfs` in your terminal to get the installation path. It should output something of the form `<some-path>/git-lfs`.
