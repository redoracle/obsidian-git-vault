...
- [Example: Document Changes](https://codemirror.net/examples/change/)
- [Example: Configuration and Extension](https://codemirror.net/examples/config/)
...
1. Every new editor pane in Obsidian subscribes itself
   by its filepath ([LineAuthoringSubcriber](../../src/editor/control.ts))
   and listens in an internal publish-subscriber-model
   ([eventsPerFilepath.ts](../../src/editor/eventsPerFilepath.ts))
   for updates on that filepath.
2. Any changed file in the Obsidian Vault or anytime when a new
   file is opened, [lineAuthorProvider](../../src/editor/lineAuthor/lineAuthorProvider.ts)
   initiates the asynchronous computation of the
   [LineAuthoring](../../src/editor/lineAuthor/model.ts)
   via [simpleGit.ts](../../src/gitManager/simpleGit.ts) -
...
4. The notified `LineAuthoringSubcriber` creates a new transaction
   (via [newComputationResultAsTransaction](../../src/editor/lineAuthor/model.ts))
...
6. The [StateField's update](https://codemirror.net/docs/ref/#state.StateField^define^config.update)
   ...
   The [lineAuthorState](../../src/editor/lineAuthor/model.ts) updates itself with the
   newest `LineAuthoring`, if it one was provided in the transaction.
7. The [lineAuthorGutter](../../src/editor/lineAuthor/view/view.ts) is automatically re-rendered,
...
- open file in a large, complex real-world vault with unknown characteristics
  ...
  - We expect no errors, but after adding the unsaved changed gutter update feature,
        an early bug was present, where errors would occur during rendering and the view would become messed up.
- UI should render correctly regardless of whether line numbers are shown as well or not.
  - [see obsidian forum discussion](https://forum.obsidian.md/t/added-editor-gutter-overlaps-and-obscures-editor-content/45217)
...
1. Make multi-line insertions, deletions and in-line changes (e.g. via cut-copy-pasting of blocks of text).

- Hint: Use Ctrl+Z as well.
- The behavior should be same as above.

1. Make changes at the intersection of unsaved and saved changes. The result should be consistent with above.
