...
You can either start by cloning an existing remote repository as described [[#For existing remote repository|in the For existing remote repository section]] or start with initializing a new repository locally and optionally push that to a remote repository as described [[#Create new local repository|in the Create new local repository section]].
...
The Git implementation on mobile is **very unstable**. Using native Git on mobile is not recommended; consider other syncing approaches.

One alternative is [GitSync](https://github.com/ViscousPot/GitSync), which is available on both Android and iOS. It is not associated with this plugin. A [GitSync setup tutorial](https://viscouspotenti.al/posts/gitsync-all-devices-tutorial) is available for all devices.
