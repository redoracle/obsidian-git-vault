...
#### Credential helpers and migration note

As of the 1.0 release the plugin no longer ships or writes an embedded askpass helper (`obsidian_askpass.sh`) and does not set `SSH_ASKPASS` at runtime. Desktop Git authentication now relies on system credential helpers. If you previously relied on the plugin-provided askpass flow, migrate to a system helper listed below.

Recommended helpers:

- **Git Credential Manager (GCM)** — cross-platform: <https://aka.ms/gcm>
- **macOS Keychain helper** — `git-credential-osxkeychain` (installed with Git for mac)
- **ssh-agent** / platform SSH agent — for SSH key-based authentication

Quick troubleshooting steps:

1. Verify your credential helper: `git config --global credential.helper`
2. Check SSH agent keys: `ssh-add -l` (start/enable `ssh-agent` if empty)
3. On macOS, ensure the Keychain helper is enabled or install GCM (`brew install --cask git-credential-manager-core`).
4. On Windows, install and use Git Credential Manager for seamless HTTPS auth.
5. To restore an askpass-like prompt, set `SSH_ASKPASS` in your environment to an appropriate GUI helper or use your OS credential manager.

See `CHANGELOG.md` and `src/gitManager/simpleGit.ts` for the inline migration note and further context.
