...
export default class Tools {
            ...
            for (const f of files) {
                ...
                if (over100mb) {
                    ...
                    if (this.plugin.useSimpleGit) {
                        isFileTrackedByLfs = await (
                            gitManager as SimpleGit
                        ).isFileTrackedByLFS(f.path);
                    }
    ...
    async runRawCommand() {
        ...
        const desktopGitManager = gitManager as SimpleGit;
        ...
        void this.plugin.promiseQueue.addTask(async () => {
            const notice = this.plugin.showNotice(
                `Running '${command}'...`,
                999_999
            );

            try {
                const res = await desktopGitManager.rawCommand(command);
                if (res) {
