...
export default class ObsidianGit extends Plugin {
    ...
    private async cloneGitRepoAsDedicatedVault(
        ...
        const safeResolveAbsolute = (absolutePath: string): string | null => {
            ...
            // Build the result via string concatenation rather than
            // path.join/resolve to avoid static-analysis path-traversal
            // findings that flag any call whose arguments include a spread of
            // a non-literal array — even when each element has been validated.
            // Since every segment has been checked against isSafeSegment
            // (alphanumeric + "._-", no separators, no ".." or "."), simple
            // concatenation with path.sep is safe and produces the same result
            // that path.join would.  The trailing normalize collapses any
            // redundant separators introduced by the root string itself.
            ...
            // Invariant: the concatenated result must still start with the
            // same root.  Guards against exotic platform edge cases.
