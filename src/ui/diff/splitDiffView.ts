...
export default class SplitDiffView extends ItemView {
    ...
    async gitShow(commitHash: string, file: string): Promise<string> {
        const maybeGM = this.plugin.gitManager as unknown;
        if (!this.plugin.useSimpleGit) {
            // Non-SimpleGit managers do not expose `show`; treat as "not found".
            return "";
        }
        if (!maybeGM || typeof (maybeGM as SimpleGit).show !== "function") {
            return "";
        }

        try {
            return await (maybeGM as SimpleGit).show(commitHash, file, false);
        } catch (error) {
            if (error instanceof GitError) {
                if (
                    ...
                    // Occurs when trying to run diff with an object that's actually a nested repository
    ...
    renderButtons(): HTMLElement {
        ...
        stageButton.onmousedown = async (_) => {
            ...
            try {
                type AppliesPatch = {
                    applyPatch: (p: string) => Promise<void>;
                };
                const maybeGM = this.plugin.gitManager as unknown;
                if (
                    maybeGM &&
                    typeof (maybeGM as AppliesPatch).applyPatch === "function"
                ) {
                    await (maybeGM as AppliesPatch).applyPatch(patch);
                } else {
                    this.plugin.displayError(
                        "applyPatch is not supported by the current git manager"
                    );
                    return;
                }
