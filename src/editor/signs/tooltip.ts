...
function createTooltip(
    ...
    stageBtn.onclick = async () => {
        ...
        if (!plugin) return;
        try {
            await plugin.promiseQueue.addTask(() =>
                plugin.hunkActions.stageHunk(pos)
            );
            togglePreviewHunk(editor, pos);
        } catch (err) {
    ...
    resetBtn.onclick = async () => {
        ...
        try {
            await plugin.promiseQueue.addTask(() =>
                // `resetHunk` is synchronous (returns void); wrap in a resolved
                // Promise so it satisfies the `addTask` contract.
                Promise.resolve(plugin.hunkActions.resetHunk(pos))
            );
            togglePreviewHunk(editor, pos);
        } catch (err) {
            plugin.displayError(err);
            console.error("Failed to reset hunk:", err);
        }
    };
