...
import { parseColoringMaxAgeDuration } from "src/setting/settingsHelpers";
...
export type LineAuthoring = Blame | "untracked" | "failed";

export type RenderableLineAuthoring = Exclude<
    LineAuthoring,
    "untracked" | "failed"
>;
...
export function lineAuthoringFailureId(
    path: string
): LineAuthoringId | undefined {
    if (path === undefined) {
        return undefined;
    }
    return `failed-path${path}`;
}
