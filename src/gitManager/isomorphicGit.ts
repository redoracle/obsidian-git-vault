...
export class IsomorphicGit extends GitManager {
    ...
    getRepo(): {
        ...
        return {
            ...
            onAuth: () => {
                ...
                const password =
                    this.plugin.providerSecrets.getGitHttpPassword() ??
                    this.plugin.localStorage.getPassword();
                return {
            ...
            onAuthFailure: async () => {
                ...
                if (username) {
                    ...
                    if (password) {
                        if (this.plugin.providerSecrets.isSupported()) {
                            this.plugin.providerSecrets.setGitHttpUsername(
                                username
                            );
                            this.plugin.providerSecrets.setGitHttpPassword(
                                password
                            );
                            this.plugin.providerSecrets.clearLegacyGitHttpCredentialCopies();
                        } else {
    ...
    async status(opts?: { path?: string }): Promise<Status> {
        let notice: INoticeHandle | undefined;
        const timeout = window.setTimeout(() => {
            notice = this.plugin.showNotice(
        ...
        try {
            ...
            if (hadConflict) {
                ...
                if (!branchInfo.current || !branchInfo.tracking) {
                    throw new Error(
                        "Cannot commit conflict resolution without a local branch and upstream branch."
                    );
                }
    ...
    async pull(): Promise<FileStatusResult[]> {
        ...
        try {
            ...
            if (!branchInfo.current || !branchInfo.tracking) {
                throw new Error(
                    "Pull requires a local branch with an upstream branch."
                );
            }
    ...
    async push(): Promise<number> {
        ...
        try {
            ...
            if (!trackingBranch || !currentBranch) {
                throw new Error(
                    "Push requires a local branch with an upstream branch."
                );
            }
    ...
    async branchInfo(): Promise<BranchInfo & { remote: string }> {
        try {
            ...
            if (
                ...
                error.message.includes("Could not find HEAD")
            ) {
                throw Object.assign(
                    new Error("Repository has no HEAD / no branches"),
                    { cause: error }
                );
            }
            this.plugin.displayError(error);
    ...
    async getRemoteBranchesFromUrl(url: string): Promise<string[]> {
        ...
        try {
            ...
            throw new Error(
                "Embedded Git remote checks require an absolute HTTP(S) URL."
            );
        }

        if (parsedUrl.protocol !== "http:" && parsedUrl.protocol !== "https:") {
            throw new Error(
                "Embedded Git remote checks require an HTTP(S) remote URL."
            );
        }
        ...
        try {
            const refs = await git.listServerRefs({
                http: repo.http,
                onAuth: repo.onAuth,
                onAuthFailure: repo.onAuthFailure,
            ...
            // Surface network/server/auth failures with a clear UI message
            this.plugin.displayError(error as Error);
            throw error;
        }
    }
    ...
    private showNotice(
        message: string,
        infinity = true
    ): INoticeHandle | undefined {
        if (!this.plugin.settings.disablePopups) {
            return this.plugin.showNotice(
                message,
