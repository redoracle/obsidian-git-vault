...
export abstract class GitManager {
    ...
    /**
     * Return the list of remote branches for a configured remote name in the
     * current repository (for example, `origin`). Implementations should use
     * the locally-configured remote rather than accepting an arbitrary URL.
     *
     * Typical usage: UI dropdowns that list branches for a named remote that
     * is already present in the local repo. Authentication and transport will
     * follow the repository's configured credential helpers/settings.
     *
     * @param remote - The configured remote name (e.g. "origin").
     */
    abstract getRemoteBranches(remote: string): Promise<string[]>;

    /**
     * Return the list of branches for a repository referenced by a raw URL.
     * Unlike {@link getRemoteBranches}, this method accepts a repository URL
     * (HTTP(S) or other supported transport) and attempts to enumerate heads
     * directly from the remote server (for example via `ls-remote` or
     * the protocol-specific listing API).
     *
     * Typical usage: settings screens where a user has pasted a remote URL
     * that is not yet configured locally. Implementations may need to
     * perform an unauthenticated query or prompt for credentials; behaviour
     * (and allowed protocols) is implementation-specific.
     *
     * @param url - Absolute repository URL used to query available branches.
     */
    abstract getRemoteBranchesFromUrl(url: string): Promise<string[]>;
    ...
    async formatCommitMessage(template: string): Promise<string> {
        ...
        template = template.replace(
            "{{date}}",
            moment.utc().local().format(this.plugin.settings.commitDateFormat)
        );
