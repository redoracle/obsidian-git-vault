...
export class SimpleGit extends GitManager {
    ...
    async setGitInstance(ignoreError = false): Promise<void> {
        if (await this.isGitInstalled()) {
            ...
            if (this.plugin.settings.basePath) {
                ...
                if (exists) {
                    ...
                    this.plugin.showNotice(
                        "ObsidianGit: Base path does not exist"
                    );
                }
            }
            ...
            // simple-git (via @simple-git/argv-parser) blocks a set of env
            // variables that could spawn interactive programs or modify git
            // config in unsafe ways, unless the corresponding allowUnsafe*
            // option is explicitly enabled.
            //
            // Since the plugin never needs interactive paging, editors, or
            // askpass prompts – and because git does not invoke pagers or
            // editors when stdout is a pipe (which it always is here) – the
            // safest fix is to remove these keys from the inherited env
            // rather than enabling the unsafe flags.
            //
            // Complete list sourced from @simple-git/argv-parser v1.1.x `y`
            // map; add new entries here as simple-git adds more.
            const SIMPLE_GIT_BLOCKED_ENV: ReadonlyArray<string> = [
                "EDITOR",
                "GIT_ASKPASS",
                "GIT_CONFIG",
                "GIT_CONFIG_COUNT",
            ...
            for (const key of SIMPLE_GIT_BLOCKED_ENV) {
                delete envs[key];
            }
            ...
            /**
             * Migration note: the plugin no longer provides an askpass helper
             * or the runtime `obsidian_askpass.sh` script, and the plugin does
             * not set `SSH_ASKPASS`. Credential prompts should be provided by
             * the system or a platform credential helper instead.
             *
             * Recommended helpers:
             *  - Git Credential Manager (cross-platform): https://aka.ms/gcm
             *  - macOS Keychain helper: `git-credential-osxkeychain`
             *  - `ssh-agent` / platform SSH agent for SSH key authentication
             *
             * Troubleshooting:
             *  - Verify your credential helper: `git config --global credential.helper`
             *  - Check SSH agent keys: `ssh-add -l` (start/enable ssh-agent if empty)
             *  - On macOS install or enable the Keychain helper, or install GCM
             *    via Homebrew (`brew install --cask git-credential-manager-core`).
             *  - On Windows, use Git Credential Manager (GCM) or the system
             *    credential store provided by Git for Windows.
             *  - To restore an askpass-like flow, set `SSH_ASKPASS` in your
             *    environment to a GUI helper or use your OS credential manager.
             *
             * This corresponds to removing the embedded askpass script and the
             * `SSH_ASKPASS` handling from the plugin. See CHANGELOG.md and
             * README.md for migration guidance and troubleshooting.
             */
            envs["OBSIDIAN_GIT"] = "1";
    ...
    getRelativeVaultPath(filePath: string): string {
        ...
        // Used only to compute a relative display path; the joined path is not dereferenced.
    ...
    // askpass helper removed; no runtime script handling in this implementation.
    ...
    async getSubmodulePaths(): Promise<string[]> {
        return new Promise<string[]>((resolve) => {
            this.git.outputHandler((_cmd, stdout, _stderr, args) => {
                ...
                if (!(args.includes("submodule") && args.includes("foreach"))) {
                    return;
                }
                ...
                if (
                    ...
                    try {
                        ...
                        const errMessage =
                            err instanceof Error ? err.message : String(err);
                        this.plugin.displayError(
                            `Pull failed (${this.plugin.settings.syncMethod}): ${errMessage}`
                        );
                        return;
                    }
                    ...
                    try {
                        ...
                        const errMessage =
                            err instanceof Error ? err.message : String(err);
                        this.plugin.displayError(
                            `Sync failed (${this.plugin.settings.syncMethod}): ${errMessage}`
                        );
                    }
                }
                this.app.workspace.trigger("obsidian-git:head-change");
    ...
    async getRemoteUrl(remote: string): Promise<string | undefined> {
        try {
            ...
            if (String(error).includes(remote)) {
                return undefined;
            } else {
                throw error;
            }
            ...
            diff: {
                ...
                files:
                    e.diff?.files.map<DiffFile>((f) => {
                        const status =
                            "status" in f && typeof f.status === "string"
                                ? f.status
                                : "M";
                        const fromPath =
                        ...
                        return {
                            ...
                            fromVaultPath:
                                fromPath != undefined
                                    ? this.getRelativeVaultPath(fromPath)
                                    : undefined,
                            hash: e.hash,
                            status,
                            binary: f.binary,
    ...
    /**
     * Replace the commit history on the specified branch with a single new commit
     * representing the current working tree. This is destructive and irreversible.
     * Only supported when using the native Git binary (simple-git).
     *
     * @param branch    Branch name to flatten; defaults to the current branch.
     * @param forcePush When true, a `git push --force` is attempted after the
     *                  rewrite to overwrite the remote history as well.
     */
    async flattenBranch(branch?: string, forcePush = false): Promise<void> {
        const branchInfo = await this.branchInfo();
        const current = branch ?? branchInfo.current;
        if (!current) throw new Error("No branch selected");

        const tempBranch = `${current}-obsidian-flatten-${Date.now()}`;
        let onOrphanBranch = false;
        let rewrittenLocally = false;
        let attemptedForcePush = false;
        try {
            this.plugin.setPluginState({ gitAction: CurrentGitAction.commit });

            // Create an orphan branch and commit current working tree as a single root commit
            await this.git.checkout(["--orphan", tempBranch]);
            onOrphanBranch = true;

            await this.git.add(["-A"]);
            const msg = `Flattened history by Obsidian Git on ${new Date().toISOString()}`;
            await this.git.commit(msg);

            // Move original branch pointer to the new commit and switch back.
            await this.git.branch(["-f", current, tempBranch]);
            rewrittenLocally = true;
            await this.git.checkout([current]);
            onOrphanBranch = false;
            ...
            if (forcePush) {
                this.plugin.setPluginState({
                    gitAction: CurrentGitAction.push,
                });
                const status = await this.git.status();
                if (status.tracking) {
                    const [remote, ...branchParts] = status.tracking.split("/");
                    const remoteBranch = branchParts.join("/") || current;
                    attemptedForcePush = true;
                    await this.git.push([
                        "--force",
                        remote,
                        `${current}:${remoteBranch}`,
                    ]);
                    this.app.workspace.trigger("obsidian-git:head-change");
                } else {
                    this.plugin.log(
                        "flattenBranch: no tracking branch configured — skipping force push"
                    );
                }
            }
        } catch (error) {
            if (onOrphanBranch) {
                try {
                    ...
                    // Log recovery failure with context so it can be debugged later.
                    try {
                        this.plugin.log(
                        ...
                        // Ensure logging problems don't mask the original error
                        console.error(
            ...
            await this.cleanupTempBranch(tempBranch, error, current);

            if (rewrittenLocally && attemptedForcePush) {
                const rootMessage =
                    error instanceof Error ? error.message : String(error);
                const wrapped = new Error(
                    `History was flattened locally, but force push failed: ${rootMessage}`
                );
                this.plugin.displayError(wrapped.message);
                throw wrapped;
            }
            ...
            if (rewrittenLocally && !attemptedForcePush) {
                const rootMessage =
                    error instanceof Error ? error.message : String(error);
                const wrapped = new Error(
                    `History was flattened locally, but cleanup (temp branch deletion or recovery) failed: ${rootMessage}`
                );
                this.plugin.displayError(wrapped.message);
                throw wrapped;
            }
    ...
    private async cleanupTempBranch(
        tempBranch: string,
        originalError: unknown,
        current: string
    ): Promise<void> {
        const logCleanupFailure = (deleteError: unknown): void => {
            try {
                this.plugin.log(
        ...
        try {
            ...
            if (Object.keys(branchSummary.branches).includes(tempBranch)) {
                try {
                    await this.git.branch(["-D", tempBranch]);
                } catch (deleteError) {
                    logCleanupFailure(deleteError);
                }
            } else {
                try {
                    this.plugin.log(
                        `flattenBranch: temp branch '${tempBranch}' not found during cleanup; skipping deletion`
                    );
                } catch (logError) {
                    console.error(
    ...
    async branchIsMerged(branch: string): Promise<boolean> {
        ...
        return !notMergedBranches.all.includes(branch);
    }
    ...
    async clone(url: string, dir: string, depth?: number): Promise<void> {
        ...
        const cloneTarget = this.resolveVaultChildPath(vaultPath, dir);
    ...
    private resolveVaultChildPath(
        ...
        // Validated immediately below with path.relative to keep the target inside the vault.
        // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal
        const normalizedVaultPath = path.resolve(vaultPath);
        // Validated immediately below with path.relative to keep the target inside the vault.
        // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal
        const targetPath = path.resolve(normalizedVaultPath, childPath);
        const relativePath = path.relative(normalizedVaultPath, targetPath);
        if (relativePath.startsWith("..") || path.isAbsolute(relativePath)) {
            throw new Error(`Refusing to clone outside vault: ${childPath}`);
        }
        return targetPath;
    }
    ...
    async getRemoteBranchesFromUrl(url: string): Promise<string[]> {
        try {
            const output = await this.git.raw(["ls-remote", "--heads", url]);
    ...
    /**
     * Returns the configured default branch for a remote (for example, "main")
     * by resolving refs/remotes/<remote>/HEAD.
     */
    async getRemoteDefaultBranch(remote: string): Promise<string | undefined> {
        try {
            ...
            const [, branch] = splitRemoteBranch(symbolic);
            return branch;
        } catch {
            return undefined;
        }
    ...
    /**
     * Returns branch context when the current tracked branch is not the remote
     * default branch. This is useful to explain GitHub/GitLab compare/PR prompts.
     */
    async getNonDefaultTrackingContext(): Promise<
        | {
    ...
    > {
        const status = await this.git.status();
        if (!status.current || !status.tracking) return undefined;
        const [remote] = splitRemoteBranch(status.tracking);
        if (!remote) return undefined;
        const remoteDefaultBranch = await this.getRemoteDefaultBranch(remote);
        if (!remoteDefaultBranch || status.current === remoteDefaultBranch) {
            return undefined;
        }
        return {
    ...
    private isNetworkFailure(errorOrMessage: unknown): boolean {
        const message =
        ...
        return (
            ...
            /ssh: connect to host .*? port .*?: Operation timed out/.test(
                message
            ) ||
            /ssh: connect to host .*? port .*?: Network is unreachable/.test(
                message
            ) ||
            /ssh: connect to host .*? port .*?: Undefined error: 0/.test(
                message
            )
        );
    }
