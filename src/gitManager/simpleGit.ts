...
export class SimpleGit extends GitManager {
    ...
    async askpass(): Promise<void> {
        ...
        try {
            ...
            for await (const event of watcher) {
                ...
                let notice: ReturnType<ObsidianGit["showNotice"]> | undefined;
                // The text is too long for the modal, so a notice is shown instead
                if (data.length > 60) {
                    notice = this.plugin.showNotice(data, 999_999);
                }
    ...
    /**
     * Excludes the generated askpass script from the local repository.
     *
     * The script is recreated by the plugin and marked executable at runtime.
     * Keeping it out of Git avoids noisy permission changes across devices.
     */
    async addAskPassScriptToExclude(): Promise<void> {
        try {
            ...
            const contains = lines.some((line) =>
                line.includes(repoRelativeAskPassScriptFile)
            );
            if (!contains) {
    ...
    async flattenBranch(branch?: string, forcePush = false): Promise<void> {
        ...
        try {
            ...
            if (rewrittenLocally && attemptedForcePush) {
                ...
                this.plugin.displayError(wrapped);
                throw wrapped;
            }
