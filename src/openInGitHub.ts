...
function isSimpleGitManager(manager: GitManager): manager is SimpleGit {
    return "getSubmoduleOfFile" in manager && "git" in manager;
}
...
export async function openLineInGitHub(
    ...
    if (isGitHub) {
        ...
        showNotice("It seems like you are not using GitHub");
    }
}

export async function openHistoryInGitHub(
    file: TFile,
    manager: GitManager,
    showNotice: (message: string, timeout?: number) => void
) {
