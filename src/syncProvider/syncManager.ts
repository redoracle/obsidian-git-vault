...
export class SyncManager {
    ...
    private configDirPollInterval: number | undefined;
    private configDirSnapshot: Map<string, number> = new Map();
    private configDirBaselineInitialized = false;
    private configDirInitGen = 0;
    ...
    private registerSmartTriggers(): void {
        ...
        if (s.syncOnFileChange && s.activeSyncProvider !== "git") {
            ...
            // creating a duplicate interval or overwriting state.
            const gen = ++this.configDirInitGen;
            // Attempt an initial snapshot before enabling the poll interval
            // so the first comparison has a real baseline. If the initial
            // snapshot fails, start the interval anyway — the first poll
            // will establish the baseline without triggering a spurious
            // change because `pollConfigDir` guards when the baseline is
            // not yet initialized.
            this.snapshotConfigDir()
                .then((baseline) => {
                    if (gen !== this.configDirInitGen) return;
                    this.configDirSnapshot = baseline;
                    this.configDirBaselineInitialized = true;
                    this.configDirPollInterval = window.setInterval(() => {
                        void this.pollConfigDir();
                    }, 5_000);
                })
                .catch((err) => {
                    if (gen !== this.configDirInitGen) return;
                    console.debug(
                        "[Git Vault] initial snapshotConfigDir failed, starting poll to establish baseline:",
                        err
                    );
                    this.configDirPollInterval = window.setInterval(() => {
                        void this.pollConfigDir();
    ...
    /**
     * Build a mtime fingerprint of every file under the vault config dir
     * (e.g. `.obsidian`).  Returns a Map<vaultPath, mtime>.
     */
    private async snapshotConfigDir(): Promise<Map<string, number>> {
        const adapter = this.plugin.app.vault.adapter;
        const configDir = this.plugin.app.vault.configDir;
        if (!configDir) return new Map();
            ...
            try {
                ...
                for (const filePath of res.files) {
                    try {
                        ...
                        snapshot.set(filePath, s?.mtime ?? 0);
                    } catch {
    ...
    /**
     * Compare the current config-dir state against the stored snapshot.
     * Calls `notifyFileChange()` when any file is added, removed, or modified.
     */
    private async pollConfigDir(): Promise<void> {
        const next = await this.snapshotConfigDir();
        // If we haven't yet established an initial baseline, use this
        // poll to establish it and avoid emitting a spurious change.
        if (!this.configDirBaselineInitialized) {
            this.configDirSnapshot = next;
            this.configDirBaselineInitialized = true;
            return;
        }

        let changed = next.size !== this.configDirSnapshot.size;
        if (!changed) {
            for (const [path, mtime] of next) {
                if (this.configDirSnapshot.get(path) !== mtime) {
                    changed = true;
                    break;
                }
            }
        ...
        this.configDirSnapshot = next;
        if (changed) {
            this.audit("trigger.config-dir-change", {});
    ...
    private clearSmartTriggers(): void {
        ...
        // Config-dir poll interval
        // Advance the generation counter to invalidate any in-flight
        // snapshotConfigDir() callbacks so they cannot start a new interval
        // after we've already cleared it here.
        this.configDirInitGen++;
        window.clearInterval(this.configDirPollInterval);
        this.configDirPollInterval = undefined;
        this.configDirSnapshot.clear();
        this.configDirBaselineInitialized = false;
